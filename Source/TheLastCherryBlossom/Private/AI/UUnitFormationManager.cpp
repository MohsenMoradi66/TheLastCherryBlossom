...
void UUnitFormationManager::MoveUnitsWithClustering(const TArray<AUnitCharacter*>& Units, const FVector& Goal)
{
    ...
    // reset flag و ذخیره اطلاعات خوشه جاری برای استفاده بعدی
    bFormationAlreadyAssigned = false;
    this->CurrentClusterUnits = Units;
    this->FinalGoal = Goal;
    this->FormationForward = FVector::ForwardVector; // مقدار پیش‌فرض

    // 1) خوشه‌بندی یونیت‌ها
    ...
    // ---------- مرحله نهایی آرایش در مقصد ----------
    if (Units.Num() > 0)
    {
        // محاسبه جهت آرایش بر اساس میانگین جهت همه خوشه‌ها
        FVector TotalDir(0.f);
        for (const FVector& Dir : ClusterDirections)
            TotalDir += Dir;

        this->FormationForward = TotalDir.GetSafeNormal();
        if (this->FormationForward.IsNearlyZero())
	...
	const FVector& InFormationForward)
{
	...
	// 1) ساخت اسلات‌ها بر اساس آرایش‌های از قبل تعریف‌شده
	TArray<FVector> Slots;
	BuildFormationSlots(Units, Goal, Slots, FormationForward);

	// 2) جلوگیری از برخورد اسلات‌ها
	const float MinSeparation = 70.f;
	ApplySimpleSeparation(Slots, MinSeparation);

	UE_LOG(LogTemp, Warning, TEXT("---- Unit Order BEFORE CostMatrix ----"));
	for (int32 i = 0; i < Units.Num(); i++)
	{
		UE_LOG(LogTemp, Warning, TEXT("Unit[%d] = %s   Loc=(%.1f, %.1f, %.1f)"),
			i,
			*Units[i]->GetName(),
			Units[i]->GetActorLocation().X,
			Units[i]->GetActorLocation().Y,
			Units[i]->GetActorLocation().Z
		);
	}

	UE_LOG(LogTemp, Warning, TEXT("---- Slot Order BEFORE CostMatrix ----"));
	for (int32 j = 0; j < Slots.Num(); j++)
	{
		UE_LOG(LogTemp, Warning, TEXT("Slot[%d] = (%.1f, %.1f, %.1f)"),
			j, Slots[j].X, Slots[j].Y, Slots[j].Z);
	}

	
	// 3) ساخت ماتریس هزینه
	TArray<TArray<float>> Cost;
	BuildCostMatrix(Units, Slots, Cost);

	// 4) حل assignment
	TArray<int32> Assignment;
	const int32 GreedyThreshold = 60; 
	if (Units.Num() > GreedyThreshold)
		GreedyAssign(Cost, Assignment);
	else
		Assignment = HungarianSolve(Cost);

	UE_LOG(LogTemp, Warning, TEXT("---- ASSIGNMENT ----"));
	for (int32 i = 0; i < Assignment.Num(); i++)
	{
		UE_LOG(LogTemp, Warning, TEXT("Unit %d -> Slot %d"), i, Assignment[i]);
	}


	// 5) اعمال اختصاص و حرکت به اسلات‌ها
	MoveClusterToAssignedSlots(Units, Slots, Assignment);
}
...
void UUnitFormationManager::AssignFormationToAllUnits()
{
	...
	// 5) حرکت مستقیم یونیت‌ها به اسلات‌ها
	MoveUnitsDirectlyToSlots(AllUnits, Slots, Assignment);
}
	...
	const TArray<int32>& Assignment)
{
	...
	for (int32 i = 0; i < N; ++i)
	{
		...
		// ⚡ مهم: اینجا **FlowField را خاموش نکنید**، فقط مقصد نهایی را می‌گذاریم
		// یونیت ابتدا باید FlowField را دنبال کند تا وارد FormationSphere شود
		// فقط وقتی وارد FormationSphere شد، مسیر مستقیم فعال می‌شود
		// Unit->MoveDirectlyToTarget(Slots[SlotIndex]); // حذف شد
