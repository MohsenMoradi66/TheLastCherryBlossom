    ...
    case EUnitState::Moving_Single:
        {
            ...
            if (ToTarget.Size() < 80.f)
            {
                ...
                {
                    UE_LOG(LogTemp, Warning, TEXT("[%s] SINGLE: Path finished"), *GetName());
                    // وضعیت فورمیشن هنوز تغییر نکند، صبر کنیم تا وارد کره شود
                }
            }
            else
            {
            ...
            // چک کره 500 سانتی برای رفتن به فورمیشن
            float DistToGoal = FVector::Dist(GetActorLocation(), FinalGoalLocation);
            if (DistToGoal <= FinalGoalRadius && !bReachedFormationTarget)
            {
                ...
                UE_LOG(LogTemp, Warning, TEXT("[%s] SINGLE: Entered Formation Sphere"), *GetName());
            }
    ...
    case EUnitState::Moving_Cluster:
        {
            ...
            if (DistToGoal <= FinalGoalRadius && !bReachedFormationTarget)
            {
                bReachedFormationTarget = true;
                SetUnitState(EUnitState::MovingToFormation);
                UE_LOG(LogTemp, Warning, TEXT("[%s] CLUSTER: Entered Formation Sphere"), *GetName());
            }
    ...
    case EUnitState::MovingToFormation:
        {
            ...
            if (Dist > 30.f)
            {
                AddMovementInput(ToSlot.GetSafeNormal(), 1.f);
            }
            else
            {
                GetCharacterMovement()->StopMovementImmediately();
