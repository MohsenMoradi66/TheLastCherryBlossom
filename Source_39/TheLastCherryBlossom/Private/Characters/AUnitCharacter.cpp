...
case EUnitState::MovingToGoal:
    if (bUseFlowField && ClusterFlowField)
    {
        ...
        {
            // یونیت وارد کره شد → رها کردن FlowField و شروع حرکت به آرایش
            bUseFlowField = false;
            SetUnitState(EUnitState::MovingToFormation);
            bReachedFormationTarget = false; // ریست Flag برای آرایش
...
case EUnitState::MovingToFormation:
    if (!bReachedFormationTarget)
    {
        ...
        const float StopThreshold = 5.f; // آستانه توقف دقیق
